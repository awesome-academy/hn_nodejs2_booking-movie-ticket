...
export class Gration081716403480994 implements MigrationInterface {
    name = 'Gration081716403480994'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`DROP INDEX \`index_fts_movie_name\` ON \`movie\``);
        await queryRunner.query(`ALTER TABLE \`ticket\` ADD \`reason_reject\` varchar(255) NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`ticket\` DROP COLUMN \`reason_reject\``);
